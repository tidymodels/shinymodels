...
The goal of shinymodels is to launch a Shiny app given tidymodels' tuning or resampling results to make it easier to explore the model's results.
...
The Shiny app to explore the model and detect any outliers or problematic observations. In the image below, the observed and predicted values are contrasted and one sample was selected (and highlighted to be red). The residuals are also visualized against `x1` and the quadratic pattern shows that a nonlinear term should be added.  

```{r example, fig.align='center', echo = FALSE}
knitr::include_graphics(system.file(package = "shinymodels", "example.png"))
```

...
test_that("can display the selected model correctly", {
  ...
  expect_equal(class(display_selected(org, performance, preds, tuning_param, input)),
               "character")
  expect_equal(length(display_selected(org, performance, preds, tuning_param, input)),
               1)
   expect_equal(display_selected(org, performance, preds, tuning_param, input),
               "Selected model: cost = 23.2, rbf_sigma = 0.000000684")
  expect_equal(display_selected(org, performance, preds, tuning_param, input1),
               "Selected model: cost = 0.00247, rbf_sigma = 0.00931")
  expect_equal(display_selected(org, preds, performance, tuning_param, input),
              "Selected model: cost = 0.00247, rbf_sigma = 0.00931")
  expect_equal(display_selected(org, performance, preds, tuning_param, input2),
